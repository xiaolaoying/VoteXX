...
from VoteXX_auth import url_names
...
def create_user(username, password, name=None):
    from VoteXX_auth.models import User
...
def password_login_view(request):
    from VoteXX_auth.view_utils import render_template
    from VoteXX_auth.views import after
    from VoteXX_auth.models import User
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            try:
                # !!!
                # add the new user create feature
                # just for test
                # !!!
                exist = User.objects.filter(user_id=username).first()
                # if exist is None:
                if exist is None:
...
urlpatterns = [
    ...
    path('password/login', password_login_view, name=PASSWORD_LOGIN_URL_NAME),
    path('password/forgot', password_forgotten_view, name=PASSWORD_FORGOTTEN_URL_NAME)
]
