...
from VoteXX_auth import DEFAULT_AUTH_SYSTEM, ENABLED_AUTH_SYSTEMS
from VoteXX_auth.security import get_user
from VoteXX_auth.url_names import AUTH_INDEX, AUTH_START, AUTH_AFTER, AUTH_WHY, AUTH_AFTER_INTERVENTION
...
# def index(request):
#     return render(request, 'VoteXX_auth/index.html')
...
def index(request):
    ...
    if len(ENABLED_AUTH_SYSTEMS) == 1 and not user:
        return HttpResponseRedirect(
            reverse(AUTH_START, args=[ENABLED_AUTH_SYSTEMS[0]]) + '?return_url=' + request.GET.get('return_url', ''))
...
def login_box_raw(request, return_url='/', auth_systems=None):
    ...
    if auth_systems is not None:
        enabled_auth_systems = set(auth_systems).intersection(set(ENABLED_AUTH_SYSTEMS)).intersection(
...
def after(request):
    ...
    if user:
        ...
        return HttpResponseRedirect(
            "%s?%s" % (reverse(AUTH_WHY), urlencode({'system_name': request.session['auth_system_name']})))
