...
def prepare_vars(request, values):
  ...
  vars_with_user['VoteXX_STATIC'] = '/static/VoteXX/VoteXX'
  vars_with_user['TEMPLATE_BASE'] = VoteXX.TEMPLATE_BASE
...
def render_template(request, template_name, values = None, include_user=True):
  ...
  return render(request, 'VoteXX/%s.html' % template_name, vars_with_user)
